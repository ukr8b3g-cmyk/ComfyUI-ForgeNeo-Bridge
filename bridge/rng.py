...
# Adapted from wangjue520/ComfyUI-Neo-Sampler 925257cf.
# Bridge: EVERY stream including the legacy default is private; fixed fp32 draws.
# No torch.manual_seed or global numpy/torch RNG mutation.
import torch

from ..vendor.forge import rng_philox
...
def randn(cfg: RNGConfig, seed, shape, generator=None):
    ...
    if cfg.source == "CPU" or cfg.device.type == "mps":
        return torch.randn(shape, device=torch.device("cpu"), generator=(generator or cfg.default_generator), dtype=torch.float32).to(cfg.device)

    return torch.randn(shape, device=cfg.device, generator=(generator or cfg.default_generator), dtype=torch.float32)
...
def randn_local(cfg: RNGConfig, seed, shape):
    ...
    return torch.randn(shape, device=local_device, generator=local_generator, dtype=torch.float32).to(cfg.device)
...
def manual_seed(cfg: RNGConfig, seed):
    ...
    cfg.default_generator = torch.Generator(cfg.local_device).manual_seed(int(seed))
